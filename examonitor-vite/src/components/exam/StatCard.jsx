  ...
  return (
    <div className={`${themes[variant]} p-8 rounded-[40px] border-2 transition-all hover:scale-[1.02] relative overflow-hidden group shadow-lg text-right`}>
      ...
      <div className="flex items-baseline gap-3">
        <p className={`text-5xl font-black italic tabular-nums leading-none ${highlight ? 'animate-pulse' : ''}`}>{value}</p>
        {progress !== undefined && <span className="text-[11px] font-bold opacity-30 italic leading-none">{Math.round(progress)}%</span>}
      </div>
      {progress !== undefined && (
