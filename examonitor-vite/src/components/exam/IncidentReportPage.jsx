...
export default function IncidentReportPage({ examId, classrooms, onBack, availableRooms }) {
  ...
  return (
    <div className={`min-h-screen p-4 md:p-16 text-right font-sans transition-colors duration-300 ${isDark ? 'bg-[#0f172a] text-white' : 'bg-slate-50 text-slate-900'}`} dir="rtl">
      ...
      <header className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-20 gap-6">
        <div className="flex items-center gap-4 md:gap-10">
          <button
            ...
            className={`w-14 h-14 md:w-20 md:h-20 flex items-center justify-center border-2 rounded-2xl md:rounded-[25px] transition-all active:scale-90 ${isDark ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-white border-slate-200 hover:bg-slate-50 shadow-sm'
              }`}
          >
      ...
      <main className="max-w-5xl mx-auto">
        <div className={`rounded-[30px] md:rounded-[55px] shadow-2xl overflow-hidden border transition-colors duration-300 ${isDark ? 'bg-slate-900 border-white/10 shadow-black/60' : 'bg-white border-slate-200 shadow-slate-200'
          }`}>
          ...
          <form onSubmit={handleSubmit} className="p-6 md:p-20 space-y-8 md:space-y-14">
                ...
                {availableRooms && availableRooms.length > 0 ? (
                  <div className="relative">
                    <select
                      ...
                      className={`w-full border-2 md:border-4 rounded-2xl md:rounded-[30px] py-4 md:py-7 px-6 md:px-10 text-xl md:text-3xl font-black transition-all outline-none appearance-none cursor-pointer ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-100 text-slate-900'
                        }`}
                      onChange={(e) => {
                        const selectedRoom = availableRooms.find(r => String(r.room_number) === String(e.target.value));
                        setFormData({
                          ...formData,
                          roomNumber: e.target.value,
                          examId: selectedRoom ? selectedRoom.exam_id : formData.examId
                        });
                      }}
                    >
                      ...
                      {availableRooms.map(room => (
                        <option key={room.id} value={room.room_number}>
                          חדר {room.room_number} {room.examName ? `(${room.examName})` : ''}
                        </option>
                      ))}
                    </select>
                  ...
                  <input
                    ...
                    type="text"
                    value={formData.roomNumber}
                    className={`w-full border-2 md:border-4 rounded-2xl md:rounded-[30px] py-4 md:py-7 px-6 md:px-10 text-xl md:text-3xl font-black transition-all outline-none ${isDark
                ...
                <div className="flex gap-2 md:gap-5 h-16 md:h-24">
                  {[
                    ...
                    <label key={lvl.id} className="flex-1 cursor-pointer">
                      ...
                      />
                      <div className={`h-full flex items-center justify-center rounded-xl md:rounded-[25px] font-black text-xs md:text-xl uppercase tracking-widest transition-all ${isDark ? 'bg-slate-800 text-slate-500' : 'bg-slate-100 text-slate-400'
                        } ${lvl.color}`}>
                        {lvl.label}
                      </div>
                    </label>
            ...
            <div className="space-y-3 md:space-y-5">
              ...
              <textarea
                required
                rows="4"
                className={`w-full border-2 md:border-4 rounded-[25px] md:rounded-[40px] py-6 md:py-10 px-6 md:px-10 text-xl md:text-3xl font-bold transition-all outline-none resize-none leading-relaxed ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-100 text-slate-900'
                  }`}
            ...
            <div className="flex flex-col md:flex-row gap-4 md:gap-8 pt-6 md:pt-10">
              <button
                type="submit"
                className={`flex-2 py-6 md:py-10 rounded-2xl md:rounded-[30px] font-black text-lg md:text-2xl uppercase tracking-widest md:tracking-[0.2em] transition-all active:scale-[0.98] shadow-2xl flex items-center justify-center gap-4 md:gap-6 ${isDark ? 'bg-white text-slate-900 hover:bg-rose-500 hover:text-white' : 'bg-slate-900 text-white hover:bg-rose-600'
                  }`}
              >
              ...
              <button
                type="button"
                onClick={goBackToExam}
                className={`flex-1 border-2 md:border-4 py-6 md:py-10 rounded-2xl md:rounded-[30px] font-black text-sm md:text-xl uppercase tracking-widest transition-all active:scale-95 ${isDark ? 'bg-transparent border-slate-700 text-slate-500' : 'bg-white border-slate-100 text-slate-400'
                  }`}
              >
                ביטול
              </button>
