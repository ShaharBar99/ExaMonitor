...
const headerElement = (
  <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 justify-between items-center w-full py-2">
    ...
    {/* TOP ROW: Title & Menu Button (Compact on Mobile) */}
    <div className="flex items-center justify-between w-full lg:w-auto gap-3">
      <div className="flex items-center gap-3">
        <button 
          onClick={() => setIsSidebarOpen(true)}
          className={`lg:hidden p-2 rounded-xl transition-all active:scale-95 ${
            isDark ? 'bg-white/10 text-emerald-400' : 'bg-slate-100 text-slate-600'
          }`}
        >
      ...
      {/* Mobile Finish Button (Smaller on mobile to save space) */}
      <button 
    ...
    {/* BOTTOM ROW: Navigation & Timer (Combined horizontally on Mobile) */}
    <div className="flex items-center justify-between lg:justify-end w-full lg:w-auto gap-2 md:gap-4">
      
      {/* Navigation Tabs: Isolated Desktop Growth */}
      <nav className={`flex transition-all duration-300 border shadow-inner
        /* Mobile: Your original compact specs */
        p-1 rounded-xl
        /* Desktop: Massive container */
        lg:p-2 lg:rounded-[40px]
      ...
      `}>
        ...
        <button 
          ...
          className={`font-black transition-all duration-300
            ...
            /* Desktop: High-impact growth */
            lg:px-16 lg:py-6 lg:rounded-[35px] lg:text-2xl
            ${dashboardTab === 'incident' ? 'bg-rose-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-300'}
          `}
        >
          דיווח
        </button>
      ...
      {/* Timer Section: Compact on Mobile, Massive on Desktop */}
      {timers.length > 0 && (
      <div className={`flex items-center border shadow-2xl transition-all
        /* Mobile (Your original specs) */
        gap-2 p-1.5 rounded-xl 
        /* Desktop (Massive & Spacious) */
      ...
      `}>
        ...
        {/* The Clock Face */}
        <div className="
          /* Mobile: Your original 75% scale */
          scale-75 md:scale-90 
          /* Desktop: Big and centered */
          lg:scale-[1.25] lg:mx-4
        ">
          <ExamTimer 
            key={selectedTimerId} 
        ...
        {timers.map((t) => (
          <button 
            ...
            `}
          >
            {/* Mobile shorthand / Desktop full label */}
