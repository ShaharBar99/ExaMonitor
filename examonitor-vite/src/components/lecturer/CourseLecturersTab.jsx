...
export default function CourseLecturersTab({lecturers, examLecturerIds, isDark, isLoading = false, onAddLecturer,}) {
    lecturers = lecturers || [];
    ...
    <div className={`rounded-[30px] md:rounded-[50px] p-6 md:p-12 shadow-2xl overflow-hidden text-right animate-in fade-in duration-500 transition-colors ${
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-right border-collapse">
          <thead>
            <tr className={`border-b text-[10x] md:text-xs uppercase tracking-wider transition-colors ${
            ...
            {isLoading ? (
                <tr>
                  ...
                  <tr key={row.id} className={`group transition-colors ${isDark ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'}`}>
                    ...
                    <span className={`px-3 py-1.5 rounded-lg text-sm font-extrabold uppercase ${
                        row.isAssigned
                            ? (isDark
                    ...
                    {!row.isAssigned ? (
                        ...
                        onClick={() => { console.log("clicked add lecturer", row.id); onAddLecturer?.(row.id); }}
      ...
      <div className="md:hidden space-y-4">
        {isLoading ? (
            ...
            <div key={row.id} className={`p-5 rounded-2xl border transition-all ${
