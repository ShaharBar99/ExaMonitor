...
export default function AdminNavbar() {
  ...
  return (
    <div className={`flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-3 md:p-4 rounded-2xl border transition-all duration-300 ${isDark ? "bg-slate-900/80 backdrop-blur-md border-white/5" : "bg-white/90 backdrop-blur-md border-slate-200 shadow-sm"
      }`}>

      {/* Top Section: Logo & Theme Toggle (Mobile optimized) */}
      <div className="flex items-center justify-between lg:justify-start gap-3">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-colors ${isDark ? "bg-blue-500/10 border-blue-500/20 text-blue-400" : "bg-blue-600 text-white border-transparent"
            }`}>
        ...
        <button
          onClick={toggleTheme}
          type="button"
          className={`lg:hidden p-2.5 rounded-xl border transition-all active:scale-90 ${isDark
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:gap-2">
        <nav className="flex items-center justify-center sm:justify-start gap-1 overflow-x-auto no-scrollbar pb-1 sm:pb-0">
          ...
          <NavLink to="/admin/classrooms" className={getLinkClass}>חדרים</NavLink>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={toggleTheme}
            type="button"
            className={`hidden lg:block p-2 rounded-xl border transition-all hover:scale-110 ${isDark
                ? "bg-slate-800 border-white/10 text-amber-400"
